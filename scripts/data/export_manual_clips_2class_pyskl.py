...
def export_dataset(config: dict, output: Path | None = None) -> dict[str, int]:
    pose_roots = [Path(config["pose_output_root"])]
    pose_roots.extend(
        Path(path) for path in config.get("additional_pose_output_roots", [])
    )
    ...
    additional_pose_splits = {
        str(value)
        for value in config.get("additional_pose_splits", source_splits)
    }
    ...
    for root_index, pose_root in enumerate(pose_roots):
        root_splits = (
            source_splits if root_index == 0 else additional_pose_splits
        )
        for split_name in root_splits:
            ...
            for class_name, label in classes.items():
                ...
                for path in sorted(folder.rglob("*.npz")):
                    ...
                    if identifier in seen_identifiers:
                        raise RuntimeError(
                            f"Duplicate pose identifier across roots: {identifier}"
                        )
