...
def evaluate_run(timeline_path, label):
    ...
    j_acc = j_cnt / n * 100
    print(f"{label:<32} | Stroke: {s_cnt:2d}/{n} ({s_acc:5.1f}%) | Side: {side_cnt:2d}/{n} ({side_acc:5.1f}%) | JOINT: {j_cnt:2d}/{n} ({j_acc:5.1f}%)")

print("=" * 95)
print("SO SÁNH ĐỐI ĐẦU 3 CẤU HÌNH TRÊN MATCH 40 (5 PHÚT) THEO GROUND TRUTH BWF:")
print("=" * 95)
evaluate_run('work_dirs/test_match40_10m_15m_full_pipeline_epoch20/timeline.json', '1. Epoch 20 RGB + Fusion 20')
evaluate_run('work_dirs/test_match40_10m_15m_full_pipeline_epoch25/timeline.json', '2. Epoch 25 RGB + Fusion 20 (Lệch)')
evaluate_run('work_dirs/test_match40_10m_15m_pure_fusion_epoch25/timeline.json', '3. Epoch 25 RGB + Fusion 25 (Đồng bộ)')
print("=" * 95)
